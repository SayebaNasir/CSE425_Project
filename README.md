...
Our project follows a pipeline of **4 deep unsupervised generative models** for multi-genre music generation from raw MIDI data. Music sequences are represented as binary 88-key piano rolls at `fs = 16 steps/sec` with core architectures are trained end-to-end:
...
A critical finding of this work is that **Mean Squared Error (MSE) loss fails on sparse piano roll data** (sparsity about 94.7%), causing models to predict silence from epoch 1. This is resolved using **Focal Loss** with positive class weighting, which is an important fix for any system trained on binary piano roll representations.
...
| 4. Sparsity Filtering | Windows with < 2% active cells discarded |
| 5. Split Assignment | MAESTRO follows official CSV; Lakh MIDI follows random 80/10/10 |
...
| Repetition Ratio | `R = #repeated_4-grams / #total_4-grams` | 0.1–0.5 is coherent |
...
| Human Listening Score | 2.50 | **3.68** (+1.18 points) |
...
| Loss Function Fix | Identified MSE silence failure mode; resolved with Focal Loss; justified binarisation threshold for class-imbalance-suppressed note probabilities |
| Generated 5 verified MIDI samples with 50–106 notes per window |
...
| Generation | generated 10 long-form compositions across all 8 genres |
| RLHF Fine-Tuning (Task 4) | REINFORCE algorithm; composite reward note density + duration adequacy + pitch diversity; valid MIDI 2/10 to 8/10; HLS 2.50 to 3.68 |
| Human Listening Survey | Rendered MIDI to MP3 via software synthesiser; distributed to 10 participants via Google Forms; aggregated perceptual ratings across all model conditions |
...
| Variational Autoencoder (Task 2) | Probabilistic latent space (dim 128), reparameterisation trick, ELBO objective combining Focal Loss reconstruction and KL divergence |
...
| Latent Interpolation | 8-step experiment confirming smooth latent space continuity ; generated 8 genre-labelled samples with Rhythm Diversity 0.683 and Repetition Ratio 0.000 |
